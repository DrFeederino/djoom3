...
class BindWindow {
    internal class idBindWindow : idWindow {
        ...
        override fun HandleEvent(event: sysEvent_s?, updateVisuals: BooleanArray?): String? {
            ...
            if (waitingOnKey) {
                ...
                if (key == KeyInput.K_ESCAPE) {
                    idStr.Companion.snPrintf(ret, ret.capacity(), "clearbind \"%s\"", bindName.GetName())
                } else {
                    idStr.Companion.snPrintf(ret, ret.capacity(), "bind %d \"%s\"", key, bindName.GetName())
                }
                return ret.toString()
            } else {
        ...
        override fun GetWinVarByName(_name: String?, winLookup: Boolean, owner: Array<drawWin_t?>?): idWinVar? {
            return if (idStr.Companion.Icmp(_name, "bind") == 0) {
                bindName
