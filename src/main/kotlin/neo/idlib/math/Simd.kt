...
object Simd {
    ...
    //
    var p_simd: idSIMDProcessor? = null
    ...
    abstract class idSIMDProcessor {
        ...
        fun  /*VPCALL*/Memcpy(dst: Array<idAnimBlend?>?, src: Array<idAnimBlend?>?, count: Int) {
            ...
            for (i in 0 until count) {
                dst.get(i) = idAnimBlend(src.get(i))
            }
        }
...
//            }
        ...
        */
        fun CmpGE(facing: ByteArray?, planeSide: FloatArray?, f: Float, numFaces: Int) {
            ...
            i = 0
            while (i < nm) {
                facing.get(i + 0) = TempDump.btoi(planeSide.get(i + 0) >= f).toByte()
                facing.get(i + 1) = TempDump.btoi(planeSide.get(i + 1) >= f).toByte()
                facing.get(i + 2) = TempDump.btoi(planeSide.get(i + 2) >= f).toByte()
                facing.get(i + 3) = TempDump.btoi(planeSide.get(i + 3) >= f).toByte()
                i += 4
            }
            while (i < numFaces) {
                facing.get(i + 0) = TempDump.btoi(planeSide.get(i + 0) >= f).toByte()
