    ...
    class idCollisionModelManagerLocal : idCollisionModelManager() {
        ...
        override fun SetupTrmModel(trm: idTraceModel, material: idMaterial): Int {
            ...
            for (i in 0..trm.numVerts) {
                vertex?.get(i)!!.p.oSet(trm.verts[i])
            ...
            if (trm.isConvex) {
                ...
                trmBrushes.get(0)?.b!!.bounds.oSet(trm.bounds)
                // link brush at node
        ...
        override fun GetModelVertex(model: Int, vertexNum: Int, vertex: idVec3): Boolean {
            ...
            vertex.oSet(models!![model]!!.vertices?.get(vertexNum)!!.p)
            return true
        }
        ...
        override fun GetModelEdge(model: Int, edgeNum: Int, start: idVec3, end: idVec3): Boolean {
            ...
            start.oSet(models!![model]!!.vertices?.get(models!![model]!!.edges?.get(currentEdgeNum)!!.vertexNum[0])!!.p)
            end.oSet(models!![model]!!.vertices?.get(models!![model]!!.edges?.get(currentEdgeNum)!!.vertexNum[1])!!.p)
            return true
        }
...
//                        }
            ...
            if (null == trm
                || trm.bounds.oGet(1).oGet(0) - trm.bounds.oGet(0).oGet(0) <= 0.0f && trm.bounds.oGet(1)
                    .oGet(1) - trm.bounds.oGet(0).oGet(1) <= 0.0f && trm.bounds.oGet(1).oGet(2) - trm.bounds.oGet(0)
                    .oGet(2) <= 0.0f
            ) {
                if (model_rotated) {
                ...
                while (i < 3) {
                    if (tw.start.oGet(i) < tw.end.oGet(i)) {
                        tw.bounds.oSet(0, i, tw.start.oGet(i) - CollisionModel.CM_BOX_EPSILON)
                        tw.bounds.oSet(1, i, tw.end.oGet(i) + CollisionModel.CM_BOX_EPSILON)
                    } else {
                        tw.bounds.oSet(0, i, tw.end.oGet(i) - CollisionModel.CM_BOX_EPSILON)
                        tw.bounds.oSet(1, i, tw.start.oGet(i) + CollisionModel.CM_BOX_EPSILON)
                    }
                    i++
                }
                tw.extents.oSet(0, tw.extents.oSet(1, tw.extents.oSet(2, CollisionModel.CM_BOX_EPSILON)))
                tw.size.Zero()
                ...
                results.endpos.oSet(start.oPlus(end.oMinus(start).oMultiply(results.fraction)))
                results.endAxis.oSet(idMat3.getMat3_identity())
                if (results.fraction < 1.0f) {
                    // rotate trace plane normal if there was a collision with a rotated model
                    if (model_rotated) {
                        results.c.normal.oMulSet(modelAxis)
                        results.c.point.oMulSet(modelAxis)
                    }
                    results.c.point.oPluSet(modelOrigin)
                    results.c.dist += modelOrigin.oMultiply(results.c.normal)
                }
            ...
            if (tw.dir.LengthSqr() > Math_h.Square(CollisionModel.CM_MAX_TRACE_DIST)) {
                ...
                results.c.normal.oSet(getVec3_origin())
            ...
            if (trm_rotated and model_rotated) {
                val tmpAxis = trmAxis.oMultiply(invModelAxis)
                i = 0
                while (i < tw.numPolys) {
            ...
            while (i < 3) {
                if (tw.start.oGet(i) < tw.end.oGet(i)) {
                    tw.bounds.oSet(0, i, tw.start.oGet(i) + tw.size.oGet(0).oGet(i) - CollisionModel.CM_BOX_EPSILON)
                    tw.bounds.oSet(1, i, tw.end.oGet(i) + tw.size.oGet(1).oGet(i) + CollisionModel.CM_BOX_EPSILON)
                } else {
                    tw.bounds.oSet(0, i, tw.end.oGet(i) + tw.size.oGet(0).oGet(i) - CollisionModel.CM_BOX_EPSILON)
                    tw.bounds.oSet(1, i, tw.start.oGet(i) + tw.size.oGet(1).oGet(i) + CollisionModel.CM_BOX_EPSILON)
                }
                if (Math.abs(tw.size.oGet(0).oGet(i)) > Math.abs(tw.size.oGet(1).oGet(i))) {
                    tw.extents.oSet(i, Math.abs(tw.size.oGet(0).oGet(i)) + CollisionModel.CM_BOX_EPSILON)
                } else {
                    tw.extents.oSet(i, Math.abs(tw.size.oGet(1).oGet(i)) + CollisionModel.CM_BOX_EPSILON)
                }
                i++
            }
            ...
            if (tw.getContacts) {
                ...
                if (model_rotated) {
                    ...
                    while (i < tw.numContacts) {
                        ...
                        tw.contacts!!.get(i)!!.point.oMulSet(modelAxis)
                        i++
                    }
                }
                if (modelOrigin != getVec3_origin()) {
                    ...
                    while (i < tw.numContacts) {
                        ...
                        tw.contacts!!.get(i)!!.dist += modelOrigin.oMultiply(tw.contacts!![i]!!.normal)
                        i++
                    }
                }
                numContacts = tw.numContacts
...
//                        }
            ...
            if (rotation.GetAngle() >= 180.0f || rotation.GetAngle() <= -180.0f) {
                ...
                lasta = 0.0f
                a = stepa
                while (Math.abs(a) < Math.abs(maxa) + 1.0f) {
        ...
        override fun Contacts(
            ...
            numContacts = 0
            end.oSet(start.oPlus(dir.SubVec3(0).oMultiply(depth)))
        ...
        override fun DebugOutput(origin: idVec3) {
            ...
            if (CollisionModel_debug.cm_testWalk.GetBool()) {
                CollisionModel_debug.start.oSet(origin)
                CollisionModel_debug.cm_testOrigin.SetString(
                    Str.va(
                        "%1.2f %1.2f %1.2f",
                        CollisionModel_debug.start.oGet(0),
                        CollisionModel_debug.start.oGet(1),
                        CollisionModel_debug.start.oGet(2)
                    )
                )
            } else {
            ...
            bounds[sscanf.nextFloat(), sscanf.nextFloat(), sscanf.nextFloat(), sscanf.nextFloat(), sscanf.nextFloat()] =
                sscanf.nextFloat()
...
//										&bounds[1][0], &bounds[1][1], &bounds[1][2] );
            ...
            if (CollisionModel_debug.cm_testRandomMany.GetBool()) {
                ...
                i = 0
                while (i < 3) {
                    CollisionModel_debug.testend[0].oSet(
                        i,
                        CollisionModel_debug.start.oGet(i) + random.CRandomFloat() * CollisionModel_debug.cm_testLength.GetFloat()
                    )
                    i++
                }
                k = 1
                while (k < CollisionModel_debug.cm_testTimes.GetInteger()) {
                    CollisionModel_debug.testend[k].oSet(CollisionModel_debug.testend[0])
                    k++
                }
            } else {
                ...
                while (k < CollisionModel_debug.cm_testTimes.GetInteger()) {
                    i = 0
                    while (i < 3) {
                        CollisionModel_debug.testend[k].oSet(
                            i,
                            CollisionModel_debug.start.oGet(i) + random.CRandomFloat() * CollisionModel_debug.cm_testLength.GetFloat()
                        )
                        i++
            ...
            if (CollisionModel_debug.cm_testRandomMany.GetBool()) {
                ...
                while (i < 3) {
                    CollisionModel_debug.testend[0].oSet(
                        i,
                        CollisionModel_debug.start.oGet(i) + random.CRandomFloat() * CollisionModel_debug.cm_testRadius.GetFloat()
                    )
                    i++
                }
                k = 1
        ...
        private fun TranslateTrmEdgeThroughPolygon(tw: cm_traceWork_s, poly: cm_polygon_s, trmEdge: cm_trmEdge_s) {
            ...
            while (i < poly.numEdges) {
                ...
                epsPl.FromLine(
                    tw.model!!.vertices!![edge.vertexNum[0]].p.oPlus(edge.normal.oMultiply(CollisionModel.CM_CLIP_EPSILON)),
                    tw.model!!.vertices!![edge.vertexNum[1]].p.oPlus(edge.normal.oMultiply(CollisionModel.CM_CLIP_EPSILON))
                )
        ...
        private fun TranslateTrmThroughPolygon(tw: cm_traceWork_s, p: cm_polygon_s): Boolean {
            ...
            if (tw.trace.fraction < fraction && !tw.getContacts) {
                fraction = tw.trace.fraction
                endp.oSet(tw.start.oPlus(tw.dir.oMultiply(fraction)))
                // decrease bounds
                i = 0
                while (i < 3) {
                    if (tw.start.oGet(i) < endp.oGet(i)) {
                        tw.bounds.oSet(0, i, tw.start.oGet(i) + tw.size.oGet(0).oGet(i) - CollisionModel.CM_BOX_EPSILON)
                        tw.bounds.oSet(1, i, endp.oGet(i) + tw.size.oGet(1).oGet(i) - CollisionModel.CM_BOX_EPSILON)
                    } else {
                        tw.bounds.oSet(0, i, endp.oGet(i) + tw.size.oGet(0).oGet(i) - CollisionModel.CM_BOX_EPSILON)
                        tw.bounds.oSet(1, i, tw.start.oGet(i) + tw.size.oGet(1).oGet(i) - CollisionModel.CM_BOX_EPSILON)
                    }
                    i++
                }
        ...
        private fun CollisionBetweenEdgeBounds(
            ...
            dir2.oSet(bt.oMinus(tw.origin).Cross(tw.axis))
            ...
            dir1.oSet(vd.oMinus(vc).Cross(dir))
            d = dir1.oMultiply(vc)
        ...
        private fun RotateEdgeThroughEdge(
            ...
            dt.oSet(vd.oMinus(tw.origin).oMultiply(tw.matrix))
            pl2.FromLine(ct, dt)
            v0 = (pl2.oGet(0) * pl1.oGet(4) + pl2.oGet(4) * pl1.oGet(0)).toDouble()
            v1 =
                (pl2.oGet(1) * pl1.oGet(2) - pl2.oGet(2) * pl1.oGet(1) + pl2.oGet(5) * pl1.oGet(3) - pl2.oGet(3) * pl1.oGet(
                    5
                )).toDouble()
            v2 =
                (pl2.oGet(1) * pl1.oGet(5) + pl2.oGet(2) * pl1.oGet(3) + pl2.oGet(5) * pl1.oGet(1) + pl2.oGet(3) * pl1.oGet(
                    2
                )).toDouble()
            a = v0 - v2
            b = v1
        ...
        private fun RotateTrmEdgeThroughPolygon(tw: cm_traceWork_s, poly: cm_polygon_s, trmEdge: cm_trmEdge_s) {
            ...
            while (i < poly.numEdges) {
                ...
                // pluecker coordinate for epsilon expanded edge
                epsDir.oSet(edge.normal.oMultiply(CollisionModel.CM_CLIP_EPSILON + CM_PL_RANGE_EPSILON))
                epsPl.FromLine(
                    tw.model!!.vertices?.get(edge.vertexNum[0])!!.p.oPlus(epsDir),
                    tw.model!!.vertices?.get(edge.vertexNum[1])!!.p.oPlus(epsDir)
                )
                ...
                if (f1 < 0.0f && f2 > 0.0f || f1 > 0.0f && f2 < 0.0f) {
                    ...
                    // collide with the epsilon expanded edge
                    epsDir.oSet(edge.normal.oMultiply(CollisionModel.CM_CLIP_EPSILON))
                    if (!RotateEdgeThroughEdge(
                            tw,
                            trmEdge.plzaxis,
                            v1.p.oPlus(epsDir),
                ...
                origin.oSet(tw.origin.oPlus(tw.axis.oMultiply(tw.axis.oMultiply(collisionPoint.oMinus(tw.origin)))))
        ...
        private fun RotatePointThroughPlane(
            ...
            d = (plane.oGet(3) + plane.Normal().oMultiply(tw.origin)).toDouble()
            normal.oSet(plane.Normal().oMultiply(tw.matrix))
            v0 = normal.oGet(2) * p.oGet(2) + d
            v1 = (normal.oGet(0) * p.oGet(1) - normal.oGet(1) * p.oGet(0)).toDouble()
            v2 = (normal.oGet(0) * p.oGet(0) + normal.oGet(1) * p.oGet(1)).toDouble()
            a = v0 - v2
            b = v1
            c = v0 + v2
            if (a == 0.0) {
        ...
        private fun RotateVertexThroughTrmPolygon(
            ...
            dir.oSet(v.p.oMinus(rotationOrigin))
        ...
        private fun RotateTrmThroughPolygon(tw: cm_traceWork_s, p: cm_polygon_s): Boolean {
            ...
            while (i < p.numEdges) {
                ...
                tw.polygonRotationOriginCache[i] =
                    tw.origin.oPlus(tw.axis.oMultiply(tw.axis.oMultiply(v.p.oMinus(tw.origin))))
                i++
            }
            ...
            if (tw.pointTrace) {
                ...
                while (i < p.numEdges) {
                    ...
                    while (k < 2) {
                        ...
                        rotationOrigin.oSet(tw.polygonRotationOriginCache[i + k])
                        j = 0
                        while (j < tw.numPolys) {
                            bp = tw.polys[j]
                            if (!bp.used) {
        ...
        private fun BoundsForRotation(origin: idVec3, axis: idVec3, start: idVec3, end: idVec3, bounds: idBounds) {
            ...
            val v1 = idVec3(start.oMinus(origin).Cross(axis))
            val v2 = idVec3(end.oMinus(origin).Cross(axis))
            i = 0
            while (i < 3) {
                ...
                if (v1.oGet(i) > 0.0f && v2.oGet(i) < 0.0f || v1.oGet(i) < 0.0f && v2.oGet(i) > 0.0f) {
                    if (0.5f * (start.oGet(i) + end.oGet(i)) - origin.oGet(i) > 0.0f) {
                        ...
                        bounds.oSet(
                            1,
                            i,
                            origin.oGet(i) + idMath.Sqrt(radiusSqr * (1.0f - axis.oGet(i) * axis.oGet(i)))
                        )
                    } else {
                        bounds.oSet(
                            0,
                ...
                // expand for epsilons
                bounds.oGet(0).oMinSet(i, CollisionModel.CM_BOX_EPSILON)
                bounds.oGet(1).oPluSet(i, CollisionModel.CM_BOX_EPSILON)
                i++
            }
        }
        ...
        private fun Rotation180(
            ...
            initialTan = Math.abs(Math.tan((idMath.PI / 360.0f * tw.angle).toDouble()).toFloat())
            ...
            tw.axis.oSet(axis)
            assert(
                tw.axis.oGet(0) * tw.axis.oGet(0) + tw.axis.oGet(1) * tw.axis.oGet(1) + tw.axis.oGet(2) * tw.axis.oGet(
                    2
                ) > 0.99f
            )
            ...
            maxErr = if (d > 0.0f) {
                (tw.radius - Math.sqrt(d.toDouble())).toFloat()
            } else {
                tw.radius
            }
            model_rotated = modelAxis.IsRotated()
            ...
            if (null == trm
                ...
                tw.vertices[0].rotationOrigin.oSet(
                    tw.origin.oPlus(
                        tw.axis.oMultiply(
                            tw.axis.oMultiply(
                                tw.vertices[0].p.oMinus(
                                    tw.origin
                                )
                            )
                        )
                ...
                tw.extents.oSet(0, tw.extents.oSet(1, tw.extents.oSet(2, maxErr + CollisionModel.CM_BOX_EPSILON)))
                ...
                if (tw.maxTan == initialTan) {
                    ...
                    results.fraction =
                        Math.abs(Math.atan(tw.maxTan.toDouble()).toFloat() * (2.0f * 180.0f / idMath.PI) / tw.angle)
                }
            ...
            while (i <= tw.numEdges) {
                ...
                val at = idVec3(edge.start.oMinus(tw.origin).oMultiply(tw.matrix))
                val bt = idVec3(edge.end.oMinus(tw.origin).oMultiply(tw.matrix))
            ...
            while (i < 3) {
                ...
                if (Math.abs(tw.size.oGet(0).oGet(i)) > Math.abs(tw.size.oGet(1).oGet(i))) {
                    tw.extents.oSet(i, Math.abs(tw.size.oGet(0).oGet(i)) + maxErr + CollisionModel.CM_BOX_EPSILON)
                } else {
                    tw.extents.oSet(i, Math.abs(tw.size.oGet(1).oGet(i)) + maxErr + CollisionModel.CM_BOX_EPSILON)
                }
                i++
            }
            ...
            endRotation.Set(rorg, axis, startAngle + (endAngle - startAngle) * results.fraction)
            endRotation.RotatePoint(results.endpos)
            results.endAxis.oSet(trmAxis.oMultiply(endRotation.ToMat3()))
            if (results.fraction < 1.0f) {
                // rotate trace plane normal if there was a collision with a rotated model
                if (model_rotated) {
                    results.c.normal.oMulSet(modelAxis)
        ...
        private fun TestTrmInPolygon(tw: cm_traceWork_s, p: cm_polygon_s, modelFeature: Int): Boolean {
            ...
            while (i <= tw.numEdges) {
                ...
                if (j >= p.numEdges) {
                    ...
                    tw.trace.c.contents = p.contents
                    tw.trace.c.material = p.material
                    tw.trace.c.point.oSet(tw.vertices[tw.edges[i].vertexNum[if (0 == flip) 1 else 0]].p)
            ...
            while (i < p.numEdges) {
                ...
                while (j < tw.numPolys) {
                    ...
                    if (k >= tw.polys[j].numEdges) {
                        ...
                        tw.trace.c.contents = p.contents
                        tw.trace.c.material = p.material
                        tw.trace.c.point.oSet(tw.model!!.vertices!![edge.vertexNum[if (0 == flip) 1 else 0]].p)
                        tw.trace.c.modelFeature = edgeNum
                        tw.trace.c.trmFeature = j
                        return true
                    }
        ...
        private fun PointNode(p: idVec3, model: cm_model_s): cm_node_s {
            ...
            while (currentNode.planeType != -1) {
                currentNode = if (p.oGet(currentNode.planeType) > currentNode.planeDist) {
        ...
        private fun TransformedPointContents(
            ...
            val p_l = idVec3(p.oMinus(origin))
        ...
        private fun ContentsTrm(
            ...
            // add offset to start point
            if (trm_rotated) {
                val dir = idVec3(trm.offset.oMultiply(trmAxis))
                tw.start.oPluSet(dir)
                tw.end.oPluSet(dir)
            } else {
                tw.start.oPluSet(trm.offset)
        ...
        private fun RemovePolygonReferences_r(node: cm_node_s?, p: cm_polygon_s) {
            ...
            while (currentNode != null) {
                ...
                currentNode = if (p.bounds.oGet(0).oGet(currentNode.planeType) > currentNode.planeDist) {
                    currentNode.children[0]
                } else if (p.bounds.oGet(1).oGet(currentNode.planeType) < currentNode.planeDist) {
                    currentNode.children[1]
                } else {
        ...
        private fun RemoveBrushReferences_r(node: cm_node_s?, b: cm_brush_s) {
            ...
            while (currentNode != null) {
                ...
                currentNode = if (b.bounds.oGet(0).oGet(currentNode.planeType) > currentNode.planeDist) {
                    currentNode.children[0]
                } else if (b.bounds.oGet(1).oGet(currentNode.planeType) < currentNode.planeDist) {
                    currentNode.children[1]
                } else {
        ...
        private fun ReplacePolygons(
            ...
            while (true) {
                ...
                if (p1.bounds.oGet(0).oGet(currentNode.planeType) > currentNode.planeDist && p2.bounds.oGet(0)
                        .oGet(currentNode.planeType) > currentNode.planeDist
                ) {
                    currentNode = currentNode.children[0]!!
                } else if (p1.bounds.oGet(1).oGet(currentNode.planeType) < currentNode.planeDist && p2.bounds.oGet(1)
                        .oGet(currentNode.planeType) < currentNode.planeDist
                ) {
        ...
        private fun TryMergePolygons(model: cm_model_s, p1: cm_polygon_s, p2: cm_polygon_s): cm_polygon_s? {
            ...
            delta.oSet(
                model.vertices!![edge.vertexNum[Math_h.INTSIGNBITNOTSET(edgeNum)]].p.oMinus(
            ...
            if (!keep1) {
                ...
                GetEdge(
                    model,
                    model.vertices!![model.edges!![Math.abs(edgeNum1)].vertexNum[Math_h.INTSIGNBITSET(edgeNum1)]].p,
                    model.vertices!![model.edges!![Math.abs(edgeNum2)].vertexNum[Math_h.INTSIGNBITNOTSET(edgeNum2)]].p,
                    newEdgeNum1,
                    CInt(-1)
                )
                if (newEdgeNum1[0] == 0) {
            ...
            newp.bounds.oSet(p1.bounds.oPlus(p2.bounds))
            return newp
        }
        ...
        private fun MergePolygonWithTreePolygons(
            ...
            while (true) {
                ...
                currentNode = if (polygon.bounds.oGet(0).oGet(currentNode.planeType) > currentNode.planeDist) {
                    currentNode.children[0]!!
                } else if (polygon.bounds.oGet(1).oGet(currentNode.planeType) < currentNode.planeDist) {
                    currentNode.children[1]!!
                } else {
        ...
        private fun PointInsidePolygon(model: cm_model_s, p: cm_polygon_s, v: idVec3): Boolean {
            ...
            while (i < p.numEdges) {
                ...
                edge = model.edges!![Math.abs(edgeNum)]
                //
                v1.oSet(model.vertices!![edge.vertexNum[Math_h.INTSIGNBITSET(edgeNum)]].p)
                v2.oSet(model.vertices!![edge.vertexNum[Math_h.INTSIGNBITNOTSET(edgeNum)]].p)
        ...
        private fun FindInternalPolygonEdges(model: cm_model_s, node: cm_node_s, polygon: cm_polygon_s) {
            ...
            while (true) {
                ...
                currentNode = if (polygon.bounds.oGet(1).oGet(currentNode.planeType) > currentNode.planeDist) {
                    currentNode.children[0]!!
                } else if (polygon.bounds.oGet(1).oGet(currentNode.planeType) < currentNode.planeDist) {
                    currentNode.children[1]!!
                } else {
...
//	delete src;
            ...
            while (k < list.numWindings) {
                ...
                do {
                    ...
                    while (i < b.numPlanes) {
                        ...
                        if (res == Plane.SIDE_ON && list.primitiveNum >= 0 && list.normal.oMultiply(b.planes[planeNum].Normal()) > 0) {
        ...
        private fun R_ChopWindingListWithTreeBrushes(list: cm_windingList_s, node: cm_node_s) {
            ...
            while (true) {
                ...
                currentNode = if (list.bounds.oGet(0).oGet(currentNode.planeType) > currentNode.planeDist) {
                    currentNode.children[0]!!
                } else if (list.bounds.oGet(1).oGet(currentNode.planeType) < currentNode.planeDist) {
                    currentNode.children[1]!!
                } else {
        ...
        private fun WindingOutsideBrushes(
            ...
            cm_windingList!!.origin.oSet(
                cm_windingList!!.bounds.oGet(1).oMinus(cm_windingList!!.bounds.oGet(0)).oMultiply(0.5f)
            )
            ...
            cm_windingList!!.bounds.oGet(0).oMinSet(
                idVec3(
                    CHOP_EPSILON,
                    CHOP_EPSILON,
                    CHOP_EPSILON
            ...
            cm_windingList!!.bounds.oGet(1).oPluSet(
                idVec3(
                    CHOP_EPSILON,
                    CHOP_EPSILON,
                    CHOP_EPSILON
        ...
        private fun HashVec(vec: idVec3): Int {
             ...
             */
            val x: Int = (vec.oGet(0) - cm_modelBounds.oGet(0).x + 0.5).toInt() + 2 shr 2
            val y: Int = (vec.oGet(1) - cm_modelBounds.oGet(0).y + 0.5).toInt() + 2 shr 2
            val z: Int = (vec.oGet(2) - cm_modelBounds.oGet(0).z + 0.5).toInt() + 2 shr 2
        ...
        private fun GetVertex(model: cm_model_s, v: idVec3, vertexNum: CInt): Boolean {
            ...
            while (vn >= 0) {
                ...
                if (Math.abs(vert.oGet(2) - p.oGet(2)) < VERTEX_EPSILON && Math.abs(
        ...
        private fun PolygonFromWinding(
            ...
            if (material.GetCullType() == cullType_t.CT_TWO_SIDED || material.ShouldCreateBackSides()) {
                ...
                CreatePolygon(model, currentW, plane.oNegative(), material)
            }
        }
                    ...
                    while (i < p.numEdges) {
                        ...
                        if (edge.normal.oGet(0) == 0.0f && edge.normal.oGet(1) == 0.0f && edge.normal.oGet(2) == 0.0f) {
                            ...
                            if (edge.numUsers.toInt() == 1) {
                                dir.oSet(model.vertices!![edge.vertexNum[if (edgeNum < 0) 1 else 0]].p.oMinus(model.vertices!![edge.vertexNum[if (edgeNum > 0) 1 else 0]].p))
                                edge.normal.oSet(p.plane.Normal().Cross(dir))
                                edge.normal.Normalize()
                            } else {
                                // the edge is used by more than one polygon
                                edge.normal.oSet(p.plane.Normal())
                            }
                        } else {
        ...
        private fun ConvertBrushSides(model: cm_model_s, mapBrush: idMapBrush, primitiveNum: Int) {
            ...
            while (i < mapBrush.GetNumSides()) {
                ...
                while (j < mapBrush.GetNumSides() && w.GetNumPoints() != 0) {
                    ...
                    w.ClipInPlace(planes[j].oNegative(), 0f)
                    j++
                }
                if (w.GetNumPoints() != 0) {
        ...
        private fun RemapEdges(node: cm_node_s, edgeRemap: IntArray) {
            ...
            while (true) {
                ...
                while (pref != null) {
                    ...
                    while (i < p.numEdges) {
                        if (p.edges[i] < 0) {
                            p.edges[i] = -edgeRemap.get(Math.abs(p.edges[i]))
                        } else {
        ...
        private fun CollisionModelForMapEntity(mapEnt: idMapEntity): cm_model_s? {    // brush/patch model from .map
            ...
            if (brushCount != 0) {
                ...
                bounds.oGet(0).Set(-256f, -256f, -256f)
                bounds.oGet(1).Set(256f, 256f, 256f)
            }
        ...
        private fun TrmFromModel(model: cm_model_s, trm: idTraceModel): Boolean {
            ...
            trm.offset.oSet(trm.bounds.GetCenter())
        ...
        private fun WritePolygons(fp: idFile, node: cm_node_s) {
            ...
            while (pref != null) {
                ...
                fp.WriteFloatString(
                    " ) ( %f %f %f ) %f",
                    p.plane.Normal().oGet(0),
                    p.plane.Normal().oGet(1),
                    p.plane.Normal().oGet(2),
                    p.plane.Dist()
                )
                fp.WriteFloatString(
                    " ( %f %f %f )",
                    p.bounds.oGet(0).oGet(0),
                    p.bounds.oGet(0).oGet(1),
                    p.bounds.oGet(0).oGet(2)
                )
                fp.WriteFloatString(
                    " ( %f %f %f )",
                    p.bounds.oGet(1).oGet(0),
                    p.bounds.oGet(1).oGet(1),
                    p.bounds.oGet(1).oGet(2)
                )
        ...
        private fun WriteBrushes(fp: idFile, node: cm_node_s) {
            ...
            while (bref != null) {
                ...
                while (i < b.numPlanes) {
                    fp.WriteFloatString(
                        "\t\t( %f %f %f ) %f\n",
                        b.planes[i].Normal().oGet(0),
                        b.planes[i].Normal().oGet(1),
                        b.planes[i].Normal().oGet(2),
                        b.planes[i].Dist()
                    )
                    i++
                }
                fp.WriteFloatString(
                    "\t} ( %f %f %f )",
                    b.bounds.oGet(0).oGet(0),
                    b.bounds.oGet(0).oGet(1),
                    b.bounds.oGet(0).oGet(2)
                )
                fp.WriteFloatString(
                    " ( %f %f %f ) \"%s\"\n",
                    b.bounds.oGet(1).oGet(0),
                    b.bounds.oGet(1).oGet(1),
                    b.bounds.oGet(1).oGet(2),
        ...
        private fun WriteCollisionModel(fp: idFile, model: cm_model_s) {
            ...
            while (i < model.numVertices) {
                fp.WriteFloatString(
                    "\t/* %d */ ( %f %f %f )\n",
                    i,
                    model.vertices!![i].p.oGet(0),
                    model.vertices!![i].p.oGet(1),
                    model.vertices!![i].p.oGet(2)
                )
                i++
            }
            fp.WriteFloatString("\t}\n")
        ...
        private fun ParseEdges(src: idLexer, model: cm_model_s) {
            ...
            while (i < model.numEdges) {
                ...
                model.edges!![i].normal.oSet(getVec3_origin())
        ...
        private fun DrawEdge(model: cm_model_s, edgeNum: Int, origin: idVec3, axis: idMat3) {
            ...
            edge = model.edges!![Math.abs(edgeNum)]
            side = edgeNum < 0
            start.oSet(model.vertices!![edge.vertexNum[if (side) 1 else 0]].p)
            end.oSet(model.vertices!![edge.vertexNum[if (side) 0 else 1]].p)
            if (isRotated) {
                start.oMulSet(axis)
                end.oMulSet(axis)
            }
            ...
            if (CollisionModel_debug.cm_drawNormals.GetBool()) {
                mid.oSet(start.oPlus(end).oMultiply(0.5f))
                if (isRotated) {
                    end.oSet(mid.oPlus(axis.oMultiply(edge.normal)).oMultiply(5f))
                } else {
                    end.oSet(mid.oPlus(edge.normal.oMultiply(5f)))
                }
        ...
        private fun DrawPolygon(
            ...
            if (CollisionModel_debug.cm_backFaceCull.GetBool()) {
                edgeNum = p.edges[0]
                edge = model.edges!![Math.abs(edgeNum)]
                dir.oSet(model.vertices!![edge.vertexNum[0]].p.oMinus(viewOrigin))
            ...
            if (CollisionModel_debug.cm_drawNormals.GetBool()) {
                ...
                while (i < p.numEdges) {
                    edgeNum = p.edges[i]
                    edge = model.edges!![Math.abs(edgeNum)]
                    center.oPluSet(model.vertices!![edge.vertexNum[if (edgeNum < 0) 1 else 0]].p)
                    i++
                }
                ...
                if (axis.IsRotated()) {
                    center.oSet(center.oMultiply(axis).oPluSet(origin))
                        ...
                        center.oPlus( // center +
                            axis.oMultiply(p.plane.Normal()).oMultiply(5f)
                        ) // axis * p.planeNormal * 5
                    )
                } else {
                    center.oPluSet(origin)
                    end.oSet(center.oPlus(p.plane.Normal().oMultiply(5f)))
                }
            ...
            if (CollisionModel_debug.cm_drawFilled.GetBool()) {
                ...
                while (i >= 0) {
                    ...
                    winding.oPluSet(
                        origin.oPlus(
                            model.vertices!![edge.vertexNum[Math_h.INTSIGNBITSET(edgeNum)]].p.oMultiply(
                                axis
                            )
                        )
                    )
        ...
        private fun DrawNodePolygons(
            ...
            while (true) {
                ...
                currentNode =
                    if (radius != 0.0f && viewOrigin.oGet(currentNode.planeType) > currentNode.planeDist + radius) {
                        currentNode.children[0]!!
                    } else if (radius != 0.0f && viewOrigin.oGet(currentNode.planeType) < currentNode.planeDist - radius) {
                        currentNode.children[1]!!
                    } else {
