...
object AsyncClient {
    ...
    class idAsyncClient {
        ...
        fun ConnectToServer(adr: netadr_t) {
            ...
            Session.Companion.session.SetGUI(guiNetMenu, HandleGuiCommand.getInstance())
        }
        ...
        fun ConnectToServer(address: String?) {
            ...
            if (idStr.Companion.IsNumeric(address)) {
            ...
            if (0 == adr.port.toInt()) {
                adr.port = Licensee.PORT_SERVER.toShort()
            }
        ...
        fun DisconnectFromServer() {
            ...
            if (clientState.ordinal >= clientState_t.CS_CONNECTED.ordinal) {
                ...
                msg.WriteByte(CLIENT_RELIABLE.CLIENT_RELIABLE_MESSAGE_DISCONNECT.ordinal)
        ...
        fun GetServerInfo(adr: netadr_t?) {
            ...
            msg.Init(msgBuf, msgBuf.capacity())
            msg.WriteShort(MsgChannel.CONNECTIONLESS_MESSAGE_ID)
        ...
        fun GetLANServers() {
            ...
            while (i < AsyncNetwork.MAX_SERVER_PORTS) {
                broadcastAddress.port = (Licensee.PORT_SERVER + i).toShort()
            ...
            while (gameTimeResidual + clientPredictTime >= UsercmdGen.USERCMD_MSEC) {
                ...
                while (snapshotGameFrame < gameFrame) {
                    ...
                    val ret = Game_local.game.ClientPrediction(
                        clientNum,
                        userCmds.get(snapshotGameFrame and AsyncNetwork.MAX_USERCMD_BACKUP - 1),
                        lastPredictFrame
                    )
        ...
        fun SendReliableGameMessage(msg: idBitMsg?) {
            ...
            outMsg.WriteByte(CLIENT_RELIABLE.CLIENT_RELIABLE_MESSAGE_GAME.ordinal)
        ...
        fun SendVersionCheck(fromMenu: Boolean = false /*= false */) {
            ...
            msg.WriteLong(AsyncNetwork.ASYNC_PROTOCOL_VERSION)
            msg.WriteShort(sys_public.BUILD_OS_ID)
        ...
        private fun Clear() {
            ...
            while (i < AsyncNetwork.MAX_USERCMD_BACKUP) {
                ...
                while (j < AsyncNetwork.MAX_ASYNC_CLIENTS) {
                    userCmds.get(i).get(j) = usercmd_t()
                    j++
                }
                i++
            }
        ...
        private fun SendUserInfoToServer() {
            ...
            msg.Init(msgBuf, msgBuf.capacity())
            msg.WriteByte(CLIENT_RELIABLE.CLIENT_RELIABLE_MESSAGE_CLIENTINFO.ordinal)
            msg.WriteDeltaDict(info, Session.Companion.sessLocal.mapSpawnData.userInfo.get(clientNum))
            if (!channel.SendReliableMessage(msg)) {
                Common.common.Error("client.server reliable messages overflow\n")
            }
            Session.Companion.sessLocal.mapSpawnData.userInfo.get(clientNum) = info
        }
        ...
        private fun SendEmptyToServer(force: Boolean = false /* = false*/, mapLoad: Boolean = false /*= false*/) {
            ...
            msg.WriteByte(CLIENT_UNRELIABLE.CLIENT_UNRELIABLE_MESSAGE_EMPTY.ordinal)
        ...
        private fun SendPingResponseToServer(time: Int) {
            ...
            msg.WriteLong(snapshotSequence)
            msg.WriteByte(CLIENT_UNRELIABLE.CLIENT_UNRELIABLE_MESSAGE_PINGRESPONSE.ordinal)
            msg.WriteLong(time)
            channel.SendMessage(clientPort, clientTime, msg)
            while (channel.UnsentFragmentsLeft()) {
                channel.SendNextFragment(clientPort, clientTime)
        ...
        private fun SendUsercmdsToServer() {
            ...
            userCmds.get(index).get(clientNum) = UsercmdGen.usercmdGen.GetDirectUsercmd()
            userCmds.get(index).get(clientNum).gameFrame = gameFrame
            userCmds.get(index).get(clientNum).gameTime = gameTime
            ...
            msg.WriteLong(gameInitId)
            msg.WriteLong(snapshotSequence)
            msg.WriteByte(CLIENT_UNRELIABLE.CLIENT_UNRELIABLE_MESSAGE_USERCMD.ordinal)
            ...
            while (i <= gameFrame) {
                ...
                idAsyncNetwork.WriteUserCmdDelta(msg, userCmds.get(index).get(clientNum), last)
        ...
        private fun ProcessUnreliableServerMessage(msg: idBitMsg?) {
            ...
            if (msg.ReadByte() < SERVER_UNRELIABLE.values().size) {
                val id = SERVER_UNRELIABLE.values()[msg.ReadByte()]
                when (id) {
                    ...
                    SERVER_UNRELIABLE.SERVER_UNRELIABLE_MESSAGE_GAMEINIT -> {
                        ...
                        if (pureWait) {
                            ...
                            // load map
                            Session.Companion.session.SetGUI(null, null)
                    ...
                    SERVER_UNRELIABLE.SERVER_UNRELIABLE_MESSAGE_SNAPSHOT -> {
                        ...
                        while (i < AsyncNetwork.MAX_ASYNC_CLIENTS) {
                            ...
                            while (j < numUsercmds) {
                                ...
                                idAsyncNetwork.ReadUserCmdDelta(msg, userCmds.get(index).get(i), last)
                                userCmds.get(index).get(i).gameFrame = snapshotGameFrame + j
                                userCmds.get(index).get(i).duplicateCount = 0
                            ...
                            while (j < AsyncNetwork.MAX_USERCMD_BACKUP) {
                                index = snapshotGameFrame + j and AsyncNetwork.MAX_USERCMD_BACKUP - 1
                                userCmds.get(index).get(i).gameFrame = 0
                                userCmds.get(index).get(i).gameTime = 0
                                j++
                            }
                            i = msg.ReadByte()
                        }
                        ...
                        if (clientState == clientState_t.CS_CONNECTED) {
                            ...
                            assert(TempDump.NOT(Session.Companion.sessLocal.GetActiveMenu()))
                            if (idAsyncNetwork.verbose.GetInteger() != 0) {
                                Common.common.Printf(
                ...
                if (msg.ReadByte() < SERVER_RELIABLE.values().size) {
                    ...
                    when (id) {
                        SERVER_RELIABLE.SERVER_RELIABLE_MESSAGE_CLIENTINFO -> {
                            ...
                            val info = idDict(Session.Companion.sessLocal.mapSpawnData.userInfo.get(clientNum))
                        ...
                        SERVER_RELIABLE.SERVER_RELIABLE_MESSAGE_SYNCEDCVARS -> {
                            val info: idDict = Session.Companion.sessLocal.mapSpawnData.syncedCVars
                            msg.ReadDeltaDict(info, info)
                            CVarSystem.cvarSystem.SetCVarsFromDict(info)
                        ...
                        SERVER_RELIABLE.SERVER_RELIABLE_MESSAGE_DISCONNECT -> {
                            ...
                            ReadLocalizedServerString(msg, string, Lib.Companion.MAX_STRING_CHARS)
                            if (clientNum == this.clientNum) {
                                Session.Companion.session.Stop()
                                Session.Companion.session.MessageBox(
                                    msgBoxType_t.MSG_OK,
                                ...
                                Session.Companion.sessLocal.mapSpawnData.userInfo.get(clientNum).Clear()
                            }
                        }
...
//			}
        ...
        private fun ProcessChallengeResponseMessage(from: netadr_t?, msg: idBitMsg?) {
            val serverGame = CharArray(Lib.Companion.MAX_STRING_CHARS)
            val serverGameBase = CharArray(Lib.Companion.MAX_STRING_CHARS)
            ...
            msg.ReadString(serverGameBase, Lib.Companion.MAX_STRING_CHARS)
            msg.ReadString(serverGame, Lib.Companion.MAX_STRING_CHARS)
            ...
            if (idStr.Companion.Icmp(CVarSystem.cvarSystem.GetCVarString("fs_game_base"), serverGameBaseStr) != 0
                || idStr.Companion.Icmp(CVarSystem.cvarSystem.GetCVarString("fs_game"), serverGameStr) != 0
            ) {
                ...
                if (!FileSystem_h.fileSystem.HasD3XP() && (0 == idStr.Companion.Icmp(
                        serverGameBaseStr,
                        "d3xp"
                    ) || 0 == idStr.Companion.Icmp(serverGameStr, "d3xp"))
                ) {
        ...
        private fun ProcessAuthKeyMessage(from: netadr_t?, msg: idBitMsg?) {
            ...
            val read_string = CharArray(Lib.Companion.MAX_STRING_CHARS)
            var retkey: String
            ...
            var auth_msg: String? = ""
            val auth_msg2 = idStr()
            if (clientState != clientState_t.CS_CONNECTING && !Session.Companion.session.WaitingForGameAuth()) {
                Common.common.Printf("clientState != CS_CONNECTING, not waiting for game auth, authKey ignored\n")
                return
            }
            authMsg = AsyncClient.authKeyMsg_t.values()[msg.ReadByte()] //TODO:out of bounds check.
            if (authMsg == authKeyMsg_t.AUTHKEY_BADKEY) {
                valid[1] = true
                valid[0] = valid[1]
                //                key_index = 0;
                authBadStatus = AsyncClient.authBadKeyStatus_t.values()[msg.ReadByte()]
                when (authBadStatus) {
                    ...
                    authBadKeyStatus_t.AUTHKEY_BAD_MSG -> {
                        ...
                        msg.ReadString(read_string, Lib.Companion.MAX_STRING_CHARS)
                ...
                if (clientState == clientState_t.CS_CONNECTING) {
                    while (true) {
                        ...
                        if (TempDump.isNotNullOrEmpty(retkey)) {
                            if (Session.Companion.session.CheckKey(retkey, true, valid)) {
                                CmdSystem.cmdSystem.BufferCommandText(cmdExecution_t.CMD_EXEC_NOW, "reconnect")
                            } else {
                    ...
                    Session.Companion.session.CDKeysAuthReply(false, auth_msg)
                }
            } else {
                msg.ReadString(read_string, Lib.Companion.MAX_STRING_CHARS)
                CVarSystem.cvarSystem.SetCVarString("com_guid", TempDump.ctos(read_string))
                Common.common.Printf("guid set to %s\n", *read_string)
                Session.Companion.session.CDKeysAuthReply(true, null)
            }
        }
        ...
        private fun ConnectionlessMessage(from: netadr_t?, msg: idBitMsg?) {
            val str =
                CharArray(Lib.Companion.MAX_STRING_CHARS * 2) // M. Quinn - Even Balance - PB packets can go beyond 1024
            val string: String?
            msg.ReadString(str, str.size)
            string = TempDump.ctos(str)

            // info response from a server, are accepted from any source
            if (idStr.Companion.Icmp(string, "infoResponse") == 0) {
            ...
            if (win_net.Sys_CompareNetAdrBase(from, idAsyncNetwork.GetMasterAddress())) {
                // server list
                if (idStr.Companion.Icmp(string, "servers") == 0) {
                    ProcessServersListMessage(from, msg)
                    return
                }
                if (idStr.Companion.Icmp(string, "authKey") == 0) {
                    ProcessAuthKeyMessage(from, msg)
                    return
                }
                if (idStr.Companion.Icmp(string, "newVersion") == 0) {
            ...
            if (idStr.Companion.Icmp(string, "challengeResponse") == 0) {
                ProcessChallengeResponseMessage(from, msg)
                return
            }

            // connect response from the server we are connecting to
            if (idStr.Companion.Icmp(string, "connectResponse") == 0) {
            ...
            if (idStr.Companion.Icmp(string, "disconnect") == 0) {
                ProcessDisconnectMessage(from, msg)
                return
            }

            // print request from server
            if (idStr.Companion.Icmp(string, "print") == 0) {
                ProcessPrintMessage(from, msg)
                return
            }

            // server pure list
            if (idStr.Companion.Icmp(string, "pureServer") == 0) {
                ProcessPureMessage(from, msg)
                return
            }
            if (idStr.Companion.Icmp(string, "downloadInfo") == 0) {
                ProcessDownloadInfoMessage(from, msg)
            }
            if (idStr.Companion.Icmp(string, "authrequired") == 0) {
        ...
        private fun SetupConnection() {
            ...
            if (clientState == clientState_t.CS_CHALLENGING) {
                ...
                if (idAsyncNetwork.LANServer.GetBool()) {
                    ...
                    msg.WriteString(Session.Companion.session.GetCDKey(false))
                    val xpkey: String = Session.Companion.session.GetCDKey(true)
                    msg.WriteByte(if (xpkey != null) 1 else 0)
                    if (xpkey != null) {
                        msg.WriteString(xpkey)
                    }
            ...
            when (reply) {
                ...
                fsPureReply_t.PURE_MISSING -> {
                    ...
                    if (idAsyncNetwork.clientDownload.GetInteger() == 0) {
                        ...
                        dlmsg.WriteShort(MsgChannel.CONNECTIONLESS_MESSAGE_ID)
        ...
        private fun ProcessReliableMessagePure(msg: idBitMsg?) {
            ...
            Session.Companion.sessLocal.ExecuteMapChange(true)

            // upon receiving our pure list, the server will send us SCS_INGAME and we'll start getting snapshots
            FileSystem_h.fileSystem.GetPureServerChecksums(inChecksums, -1, gamePakChecksum)
            outMsg.Init(msgBuf, msgBuf.capacity())
            outMsg.WriteByte(CLIENT_RELIABLE.CLIENT_RELIABLE_MESSAGE_PURE.ordinal)
        ...
        private fun HandleGuiCommandInternal(cmd: String?): String? {
            if (0 == idStr.Companion.Cmp(cmd, "abort") || 0 == idStr.Companion.Cmp(cmd, "pure_abort")) {
        ...
        private fun HandleDownloads() {
            if (updateState == clientUpdateState_t.UPDATE_SENT && clientTime > updateSentTime + 2000) {
                ...
                if (updateState == clientUpdateState_t.UPDATE_READY) {
                    //
                    if (Session.Companion.session.MessageBox(
                        ...
                        if (!updateDirectDownload) {
                            ...
                            Session.Companion.session.DownloadProgressBox(
                                backgroundDownload,
                    ...
                    do {
                        ...
                        if (backgroundDownload.url.status == dlStatus_t.DL_DONE) {
                            ...
                            while (remainlen != 0) {
                                readlen = Lib.Companion.Min(remainlen, CHUNK_SIZE)
        ...
        private fun CheckTimeout(): Boolean {
            if (lastPacketTime > 0 && lastPacketTime + idAsyncNetwork.clientServerTimeout.GetInteger() * 1000 < clientTime) {
                Session.Companion.session.StopBox()
                Session.Companion.session.MessageBox(
                    msgBoxType_t.MSG_OK,
        ...
        private fun ProcessDownloadInfoMessage(from: netadr_t?, msg: idBitMsg?) {
            val buf = CharArray(Lib.Companion.MAX_STRING_CHARS)
            ...
            dlRequest = -1
            if (infoType == SERVER_DL.SERVER_DL_REDIRECT.ordinal) {
                msg.ReadString(buf, Lib.Companion.MAX_STRING_CHARS)
                CmdSystem.cmdSystem.BufferCommandText(cmdExecution_t.CMD_EXEC_NOW, "disconnect")
            ...
            } else if (infoType == SERVER_DL.SERVER_DL_LIST.ordinal) {
                CmdSystem.cmdSystem.BufferCommandText(cmdExecution_t.CMD_EXEC_NOW, "disconnect")
                if (dlList.Num() != 0) {
                ...
                do {
                    ...
                    if (pakDl == SERVER_PAK.SERVER_PAK_YES.ordinal) {
                        ...
                        if (pakIndex != 0 || dlChecksums.get(0) != 0) {
                            Common.common.Printf(
                                "no download offered for %s ( 0x%x )\n",
                                entry.filename,
                                dlChecksums.get(pakIndex)
                            )
                            gotAllFiles = false
                        }
                    } else {
        ...
        private fun GetDownloadRequest(
            ...
            assert(
                0 == checksums.get(count) // 0-terminated
            )
            //            if (memcmp(dlChecksums + 1, checksums, sizeof(int) * count) || gamePakChecksum != dlChecksums[ 0]) {
            if (TempDump.memcmp(dlChecksums, 1, checksums, 0, count) || gamePakChecksum != dlChecksums.get(0)) {
                val newreq = idRandom()
                dlChecksums.get(0) = gamePakChecksum
        ...
        //
        init {
            updateState = clientUpdateState_t.UPDATE_NONE
            channel = idMsgChannel()
            Clear()
            clientPort = idPort()
        }
    }
