...
//
        ...
        private fun OptimizeOpcode(op: opcode_s?, var_a: idVarDef?, var_b: idVarDef?): idVarDef? {
            ...
            if (var_a != null) {
                vectorPtr.set(var_a.value!!.getVectorPtrs())
            }
            when (indexOf(op, opcodes)) {
                OP_ADD_F -> {
                    float_c = var_a!!.value!!.floatPtr + var_b!!.value!!.floatPtr
                    type = Script_Program.type_float
                }

                OP_ADD_V -> {
                    vec_c.set(vectorPtr.plus(var_b!!.value!!.getVectorPtrs()))
                ...
                OP_SUB_V -> {
                    vec_c.set(vectorPtr.minus(var_b!!.value!!.getVectorPtrs()))
                    type = Script_Program.type_vector
                }
                ...
                OP_MUL_V -> {
                    float_c = vectorPtr.times(var_b!!.value!!.getVectorPtrs())
                    type = Script_Program.type_float
                }

                OP_MUL_FV -> {
                    vec_c.set(var_b!!.value!!.getVectorPtrs().times(var_a!!.value!!.floatPtr))
                    type = Script_Program.type_vector
                }

                OP_MUL_VF -> {
                    vec_c.set(vectorPtr.times(var_b!!.value!!.floatPtr))
                    type = Script_Program.type_vector
                }

                OP_DIV_F -> {
                    float_c = Divide(var_a!!.value!!.floatPtr, var_b!!.value!!.floatPtr)
                    type = Script_Program.type_float
                }

                OP_MOD_F -> {
                    float_c = (var_a!!.value!!.floatPtr.toInt() % var_b!!.value!!.floatPtr.toInt()).toFloat()
                    type = Script_Program.type_float
                }

                OP_BITAND -> {
                    float_c = (var_a!!.value!!.floatPtr.toInt() and var_b!!.value!!.floatPtr.toInt()).toFloat()
                    type = Script_Program.type_float
                }

                OP_BITOR -> {
                    float_c = (var_a!!.value!!.floatPtr.toInt() or var_b!!.value!!.floatPtr.toInt()).toFloat()
                    type = Script_Program.type_float
                }

                OP_GE -> {
                    float_c = btoi(var_a!!.value!!.floatPtr >= var_b!!.value!!.floatPtr).toFloat()
                    type = Script_Program.type_float
                }
                ...
                OP_AND -> {
                    float_c = btoi(var_a!!.value!!.floatPtr != 0f && var_b!!.value!!.floatPtr != 0f).toFloat()
                    type = Script_Program.type_float
                }
                ...
                OP_NOT_BOOL -> {
                    int_c = btoi(!itob(var_a!!.value!!.intPtr))
                    type = Script_Program.type_boolean
                }

                OP_NOT_F -> {
                    float_c = btoi(!itob(var_a!!.value!!.floatPtr.toInt())).toFloat()
                    type = Script_Program.type_float
                }

                OP_NOT_V -> {
                    float_c = btoi(0f == vectorPtr.x && 0f == vectorPtr.y && 0f == vectorPtr.z).toFloat()
                    type = Script_Program.type_float
                }

                OP_NEG_F -> {
                    float_c = -var_a!!.value!!.floatPtr
                    type = Script_Program.type_float
                }
                ...
                OP_INT_F -> {
                    float_c = var_a!!.value!!.floatPtr.toInt().toFloat()
                    type = Script_Program.type_float
                }
                ...
                OP_EQ_V -> {
                    float_c = btoi(vectorPtr.Compare(var_b!!.value!!.getVectorPtrs())).toFloat()
                    type = Script_Program.type_float
                }

                OP_EQ_E -> {
                    float_c = btoi(var_a!!.value!!.intPtr == var_b!!.value!!.intPtr).toFloat()
                    type = Script_Program.type_float
                }
                ...
                OP_UADD_F -> {
                    float_c = var_b!!.value!!.floatPtr + var_a!!.value!!.floatPtr
                    type = Script_Program.type_float
                }
                ...
                OP_UDIV_F -> {
                    float_c = Divide(var_b!!.value!!.floatPtr, var_a!!.value!!.floatPtr)
                    type = Script_Program.type_float
                }

                OP_UMOD_F -> {
                    float_c = (var_b!!.value!!.floatPtr.toInt() % var_a!!.value!!.floatPtr.toInt()).toFloat()
                    type = Script_Program.type_float
                }

                OP_UOR_F -> {
                    float_c = (var_b!!.value!!.floatPtr.toInt() or var_a!!.value!!.floatPtr.toInt()).toFloat()
                    type = Script_Program.type_float
                }

                OP_UAND_F -> {
                    float_c = (var_b!!.value!!.floatPtr.toInt() and var_a!!.value!!.floatPtr.toInt()).toFloat()
                    type = Script_Program.type_float
                }

                OP_UINC_F -> {
                    float_c = var_a!!.value!!.floatPtr + 1
                    type = Script_Program.type_float
                }
                ...
                OP_COMP_F -> {
                    float_c = var_a!!.value!!.floatPtr.toInt().inv().toFloat()
                    type = Script_Program.type_float
                }
        ...
        private fun EmitOpcode(op: opcode_s?, var_a: idVarDef?, var_b: idVarDef?): idVarDef? {
            val statement: Script_Program.statement_s?
            var var_c: idVarDef?
                ...
                TT_LITERAL -> {
                    ...
                    while (i < 3) {
                        ...
                        if (token2.type == TT_PUNCTUATION && token2.equals("-")) {
                            if (NOT(lex.CheckTokenType(TT_NUMBER, 0, token2).toDouble())) {
...
//				if ( ( op - opcodes == OP_STOREP_F ) && ( e.TypeDef().PointerType().Type() == ev_boolean ) ) {
        ...
        private fun ParseFunctionDef(returnType: idTypeDef?, name: String) {
            ...
            if (oldscope!!.TypeDef()!!.Inherits(Script_Program.type_object) && NOT(Icmp(name, "init").toDouble())) {
            ...
            if (oldscope.TypeDef()!!.Inherits(Script_Program.type_object) && NOT(Icmp(name, "destroy").toDouble())) {
                var superClass: idTypeDef?
