...
import neo.framework.Async.MsgChannel.MAX_MESSAGE_SIZE
...
import neo.framework.FileSystem_h.MAX_PURE_PAKS
...
object AsyncClient {
    ...
    class idAsyncClient {
        ...
        private val dlChecksums: IntArray =
            IntArray(MAX_PURE_PAKS) // 0-terminated, first element is the game pak checksum or 0
        ...
        fun ConnectToServer(adr: netadr_t) {
            ...
            guiNetMenu.SetStateString(
                "status",
                Str.va(common.GetLanguageDict().GetString("#str_06749"), win_net.Sys_NetAdrToString(adr))
            )
            ...
            if (idStr.IsNumeric(address)) {
                ...
                if (serverNum < 0 || serverNum >= serverList.Num()) {
                    Session.session.MessageBox(
                        msgBoxType_t.MSG_OK,
                        Str.va(common.GetLanguageDict().GetString("#str_06733"), serverNum),
                ...
                if (!win_net.Sys_StringToNetAdr(address, adr, true)) {
                    Session.session.MessageBox(
                        msgBoxType_t.MSG_OK,
                        Str.va(common.GetLanguageDict().GetString("#str_06734"), address),
                        common.GetLanguageDict().GetString("#str_06735"),
                        true
                    )
                    return
        ...
        fun DisconnectFromServer() {
            val msg = idBitMsg()
            val msgBuf = ByteArray(MAX_MESSAGE_SIZE)
        ...
        fun SendReliableGameMessage(msg: idBitMsg) {
            ...
            outMsg.Init(msgBuf, msgBuf.size)
        ...
        private fun Clear() {
            ...
            memset(userCmds, 0, sizeOf<usercmd_t>().toULong())
            i = 0
            ...
            memset(dlChecksums, 0, (Int.SIZE_BYTES * MAX_PURE_PAKS).toULong())
            currentDlSize = 0
            totalDlSize = 0
        }
        ...
        private fun ClearPendingPackets() {
            ...
            val msgBuf = ByteArray(MAX_MESSAGE_SIZE)
            val from = netadr_t()
            while (clientPort.GetPacket(from, msgBuf, size, msgBuf.size));
        }
                        ...
                        SERVER_RELIABLE.SERVER_RELIABLE_MESSAGE_DISCONNECT -> {
                            ...
                            if (clientNum == this.clientNum) {
                                ...
                                Session.session.MessageBox(
                                    ...
                                    common.GetLanguageDict().GetString("#str_04319"),
                                    true
                                )
...
//			}
        ...
        private fun ProcessDisconnectMessage(from: netadr_t, msg: idBitMsg) {
            ...
            Session.session.MessageBox(
                msgBoxType_t.MSG_OK,
                common.GetLanguageDict().GetString("#str_04320"),
                null,
                true
            )
            Session.session.StartMenu()
            ...
            if (opcode == SERVER_PRINT.SERVER_PRINT_GAMEDENY.ordinal) {
                if (game_opcode == allowReply_t.ALLOW_BADPASS.ordinal) {
                    retpass = Session.session.MessageBox(
                        ...
                        common.GetLanguageDict().GetString("#str_04321"),
                        string,
                        true,
                        "passprompt_ok"
                    )
            ...
            if (authMsg == authKeyMsg_t.AUTHKEY_BADKEY) {
                ...
                when (authBadStatus) {
                    ...
                    authBadKeyStatus_t.AUTHKEY_BAD_BANNED -> {
                        ...
                        auth_msg = common.GetLanguageDict().GetString(Str.va("#str_0719%1d", 6 + key_index))
                        auth_msg += "\n"
                    ...
                    authBadKeyStatus_t.AUTHKEY_BAD_INUSE -> {
                        key_index = msg.ReadByte().toInt()
                        auth_msg = common.GetLanguageDict().GetString(Str.va("#str_0719%1d", 8 + key_index))
                        auth_msg += "\n"
                        auth_msg += common.GetLanguageDict().GetString("#str_04304")
                        valid[key_index] = false
                    }
                ...
                if (clientState == clientState_t.CS_CONNECTING) {
                    while (true) {
                        ...
                        retkey = Session.session.MessageBox(
                            ...
                            auth_msg,
                            common.GetLanguageDict().GetString("#str_04325"),
                            true
                        )
                        if (TempDump.isNotNullOrEmpty(retkey)) {
                            if (Session.session.CheckKey(retkey, true, valid)) {
                                ...
                                Session.session.MessageBox(
                                    msgBoxType_t.MSG_OK,
                                    auth_msg,
                                    common.GetLanguageDict().GetString("#str_04327"),
                                    true
                                )
                                continue
                            }
        ...
        private fun ProcessPureMessage(from: netadr_t, msg: idBitMsg) {
            ...
            val msgBuf = ByteArray(MAX_MESSAGE_SIZE)
            var i: Int
            val inChecksums = IntArray(MAX_PURE_PAKS)
        ...
        private fun ValidatePureServerChecksums(from: netadr_t, msg: idBitMsg): Boolean {
            ...
            val missingChecksums = IntArray(MAX_PURE_PAKS)
            ...
            when (reply) {
                ...
                fsPureReply_t.PURE_MISSING -> {
                    ...
                    if (idAsyncNetwork.clientDownload.GetInteger() == 0) {
                        // never any downloads
                        var message = Str.va(
                            common.GetLanguageDict().GetString("#str_07210"),
                            win_net.Sys_NetAdrToString(from)
                        )
                        if (numMissingChecksums > 0) {
                            message += Str.va(
                                common.GetLanguageDict().GetString("#str_06751"),
                                numMissingChecksums,
                                checkSums
                            )
                        }
                ...
                fsPureReply_t.PURE_NODLL -> {
                    common.Printf(
                        common.GetLanguageDict().GetString("#str_07211"),
                        win_net.Sys_NetAdrToString(from)
                    )
                    CmdSystem.cmdSystem.BufferCommandText(cmdExecution_t.CMD_EXEC_NOW, "disconnect")
                    return false
        ...
        private fun HandleDownloads() {
            if (updateState == clientUpdateState_t.UPDATE_SENT && clientTime > updateSentTime + 2000) {
                ...
                if (showUpdateMessage) {
                    Session.session.MessageBox(
                        msgBoxType_t.MSG_OK,
                        common.GetLanguageDict().GetString("#str_04839"),
                        common.GetLanguageDict().GetString("#str_04837"),
                        true
                    )
                    showUpdateMessage = false
                }
                ...
                if (updateState == clientUpdateState_t.UPDATE_READY) {
                    ...
                    if (Session.session.MessageBox(
                            ...
                            updateMSG.toString(),
                            common.GetLanguageDict().GetString("#str_04330"),
                            true,
                            "yes"
                        ).isEmpty() == false
                    ) {
                        if (!updateDirectDownload) {
                            ...
                            if (backgroundDownload.url.status == dlStatus_t.DL_DONE) {
                                ...
                                if (Session.session.MessageBox(
                                        ...
                                        common.GetLanguageDict().GetString("#str_04332"),
                                        true,
                                        "yes"
                                    ).isEmpty() == false
                                ) {
                                    ...
                                    Session.session.MessageBox(
                                        msgBoxType_t.MSG_OK,
                                        Str.va(common.GetLanguageDict().GetString("#str_04333"), fullPath),
                                        common.GetLanguageDict().GetString("#str_04334"),
                                        true
                                    )
                                }
                            } else {
                                ...
                                Session.session.MessageBox(
                                    msgBoxType_t.MSG_OK,
                                    common.GetLanguageDict().GetString("#str_04335"),
                    ...
                    do {
                        ...
                        FileSystem_h.fileSystem.BackgroundDownload(backgroundDownload)
                        var dltitle: CValuesRef<ByteVar>? = CValuesRef<ByteVar>()
                        // "Downloading %s"
                        sprintf(
                            dltitle,
                            common.GetLanguageDict().GetString("#str_07213"),
                        ...
                        if (backgroundDownload.url.status == dlStatus_t.DL_DONE) {
                            ...
                            if (0 == checksum || checksum != dlList[0].checksum) {
                                ...
                                Session.session.MessageBox(
                                    msgBoxType_t.MSG_OK,
                                    Str.va(
                                        common.GetLanguageDict().GetString("#str_07214"),
                                        checksum,
                                        dlList[0].checksum
                                    ),
                                    "Download failed",
                            ...
                            common.Warning("download failed: %s", dlList[0].url)
                            if (!backgroundDownload.url.dlerror.isEmpty()) {
                            ...
                            Session.session.MessageBox(
                                msgBoxType_t.MSG_OK,
                                common.GetLanguageDict().GetString("#str_07215"),
                                common.GetLanguageDict().GetString("#str_07216"),
                                true
                            )
                            dlList.Clear()
                            return
        ...
        private fun ReadLocalizedServerString(msg: idBitMsg, out: CharArray, maxLen: Int) {
            ...
            idStr.snPrintf(
                ...
                maxLen - 1,
                "%s",
                common.GetLanguageDict().GetString(TempDump.ctos(out))
            )
        }
        ...
        private fun CheckTimeout(): Boolean {
            if (lastPacketTime > 0 && lastPacketTime + idAsyncNetwork.clientServerTimeout.GetInteger() * 1000 < clientTime) {
                ...
                Session.session.MessageBox(
                    msgBoxType_t.MSG_OK,
                    common.GetLanguageDict().GetString("#str_04328"),
                    common.GetLanguageDict().GetString("#str_04329"),
                    true
                )
                CmdSystem.cmdSystem.BufferCommandText(cmdExecution_t.CMD_EXEC_NOW, "disconnect")
                return true
        ...
        private fun ProcessDownloadInfoMessage(from: netadr_t, msg: idBitMsg) {
            ...
            if (infoType == SERVER_DL.SERVER_DL_REDIRECT.ordinal.toByte()) {
                ...
                if (TempDump.isNotNullOrEmpty(
                        Session.session.MessageBox(
                            msgBoxType_t.MSG_YESNO,
                            Str.va(common.GetLanguageDict().GetString("#str_07217"), buf),
                            common.GetLanguageDict().GetString("#str_07218"),
                            true,
                            "yes"
                        )
                    )
                ...
                if (totalDlSize == 0) {
                    ...
                    Session.session.MessageBox(
                        msgBoxType_t.MSG_OK,
                        common.GetLanguageDict().GetString("#str_07219"),
                        common.GetLanguageDict().GetString("#str_07218"),
                        true
                    )
                    return
                ...
                if (gotGame) {
                    ...
                    if (Session.session.MessageBox(
                            msgBoxType_t.MSG_YESNO,
                            common.GetLanguageDict().GetString("#str_07220"),
                            common.GetLanguageDict().GetString("#str_07221"),
                            true,
                            "yes"
                        ).isEmpty()
                    ) {
                ...
                if (!gotAllFiles) {
                    ...
                    if (TempDump.NOT(
                            Session.session.MessageBox(
                                msgBoxType_t.MSG_YESNO,
                                Str.va(common.GetLanguageDict().GetString("#str_07222"), sizeStr.toString()),
                                common.GetLanguageDict().GetString("#str_07218"),
                                true,
                                "yes"
                            )
                ...
                if (!asked && idAsyncNetwork.clientDownload.GetInteger() == 1) {
                    ...
                    if (TempDump.NOT(
                            Session.session.MessageBox(
                                msgBoxType_t.MSG_YESNO,
                                Str.va(common.GetLanguageDict().GetString("#str_07224"), sizeStr.toString()),
                                common.GetLanguageDict().GetString("#str_07218"),
                                true,
                                "yes"
                            )
                ...
                Session.session.MessageBox(
                    msgBoxType_t.MSG_OK,
                    common.GetLanguageDict().GetString("#str_07223"),
                    common.GetLanguageDict().GetString("#str_07218"),
                    true
                )
            }
        ...
        private fun GetDownloadRequest(
            ...
            if (TempDump.memcmp(dlChecksums, 1, checksums, 0, count) || gamePakChecksum != dlChecksums[0]) {
                ...
                TempDump.memcmp(dlChecksums, 1, checksums, 0, MAX_PURE_PAKS)
