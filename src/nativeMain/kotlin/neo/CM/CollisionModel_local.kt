...
import neo.CM.Companion.cm_testOrigin
import neo.CM.Companion.start
...
//										&bounds[1][0], &bounds[1][1], &bounds[1][2] );
            sscanf(cm_testBoxRotation.GetString())
            ...
            Common.common.Printf(
                ...
                min_translation,
                max_translation,
                total_translation.toFloat() / num_translation
            )
            if (cm_testRandomMany.GetBool()) {
                // if many traces in one random direction
                i = 0
            ...
            if (cm_testRotation.GetBool()) {
                ...
                vec.Normalize()
                val rotation = idRotation(getVec3_origin(), vec, cm_testAngle.GetFloat())
                timer.Clear()
                timer.Start()
                i = 0
                while (i < cm_testTimes.GetInteger()) {
                ...
                Common.common.Printf(
                    ...
                    min_rotation,
                    max_rotation,
                    total_rotation.toFloat() / num_rotation
                )
            }
            testend = emptyArray()
            sscanf.close()
        ...
        private fun FreeBrush(model: cm_model_s, brush: cm_brush_s) {
            model.numBrushes--
            (model.brushMemory -= sizeOf<cm_brush_s>() + (brush.numPlanes - 1) * sizeOf<idPlane>()).toInt()
        ...
        private fun TryMergePolygons(model: cm_model_s, p1: cm_polygon_s, p2: cm_polygon_s): cm_polygon_s? {
            ...
            memcpy(newp, p1, sizeOf<cm_polygon_s>())
